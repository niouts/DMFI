<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="tag[@name = 'see']" mode="tabular">
        <tr>
            <th><xsl:value-of select="@name"/></th>
            <td>
			<xsl:choose>
			  <xsl:when test="contains(@refers,'::')">
				<xsl:choose>
				  <xsl:when test="contains(@refers,'()')">
					<a href="{substring-before(substring(@refers,2),'::')}.html#method_{substring-before(substring-after(@refers,'::'),'()')}">
						<xsl:value-of select="@description" />
					</a>
				  </xsl:when>
				  <xsl:otherwise>
					<a href="{substring-before(substring(@refers,2),'::')}.html#property__{substring-after(@refers,'::')}">
						<xsl:value-of select="@description" />
					</a>
				  </xsl:otherwise>
				</xsl:choose>
			  ...
			  <xsl:otherwise>
				<a href="{substring(@refers,2)}.html">
                    <xsl:value-of select="@description" />
                </a>
			  </xsl:otherwise>
			</xsl:choose>
			<xsl:if test="contains(@refers,'()')">
